...
public class InventoryViewUIManager : MonoBehaviour
{
    ...
    public void UpdateIngredientUI(int index, int count)
    {
        ingredientTexts[index].text = "x " + count;
        ingredientImages[index].sprite = GameManager.Instance.IngredientSOs[index].sprite;
        ingredientImages[index].color = GameManager.Instance.IngredientSOs[index].color;
    }
